...
class TodoInput(BaseModel):
    ...
    dueTime: int = Field(None, description="截止时间，Unix 时间戳，单位毫秒，例如{}".format(int(time.time() * 1000)))
    description: str = Field(None, description="待办事项描述")
    priority: int = Field(0, description="优先级 10：较低 20：普通 30：紧急 40：非常紧急")

class ScheduleSchema(BaseModel):
    userIds: str = Field(description="用户ID")
    startTime: str = Field(None, description=f"查询开始时间，格式必须为：2020-01-01T10:15:30+08:00，当前时间为{time.strftime('%Y-%m-%dT%H:%M:%S%z', time.localtime())}")
    endTime: str = Field(None, description=f"查询结束时间，格式必须为：2020-01-01T10:15:30+08:00，当前时间为{time.strftime('%Y-%m-%dT%H:%M:%S%z', time.localtime())}")
...
class ScheduleSchemaSet_data(BaseModel):
    ...
    datetime: str = Field(description=f"日程开始时间，格式为ISO-8601的date-time格式{time.strftime('%Y-%m-%dT%H:%M:%S%z', time.localtime())}，说明（全天日程必须留空，非全天日程必须有值）")
...
class ScheduleSchemaSet_data_end(BaseModel):
    date: str = Field(description=f"日程结束日期，格式：yyyy-MM-dd，当前时间为{time.strftime('%Y-%m-%d')}，说明（全天日程：必须有值结束时间需传 T+1 例如 2024-06-01 的全天日程，开始时间为 2024-06-01，则结束时间应该写 2024-06-02。非全天日程必须留空）")
    dateTime: str = Field(description=f"日程结束时间，格式为ISO-8601的date-time格式，当前时间为{time.strftime('%Y-%m-%dT%H:%M:%S%z', time.localtime())}，说明（全天日程必须留空，非全天日程必须有值）")
...
class ScheduleSearch(BaseModel):
    timeMin: Optional[str] = Field(None, description=f"日程开始时间的最小值，格式为ISO-8601的date-time格式，可不填，说明（timeMin和 timeMax最大差值为一年），当前时间为{time.strftime('%Y-%m-%dT%H:%M:%S%z', time.localtime())}")
    timeMax: Optional[str] = Field(None, description=f"日程开始时间的最大值，格式为ISO-8601的date-time格式，可不填，说明（timeMin和 timeMax最大差值为一年），当前时间为{time.strftime('%Y-%m-%dT%H:%M:%S%z', time.localtime())}")
...
def search(query: str) -> str:
    """只有需要了解实时只有需要了解实时信息或不知道的事情的时候才会使用这个工具。"""
    serp = SerpAPIWrapper()
...
def get_info_from_local(query: str) -> str:
    ...
    condense_question_prompt = ChatPromptTemplate.from_messages([
        ("system", "给出聊天记录和最新的用户问题。可能会引用聊天记录中的上下文，提出一个可以理解的独立问题。没有聊天记录，请勿回答。必要时重新配制，否则原样返还。"),
...
def FindPreciseOrder(orginrder: str, events: object) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", f"""请根据用户的输入和查询到的日程信息，提取出与用户输入最匹配的1个日程id以及是否为全天事件。注意查询到的数据结构为：
{{'events': [{{
    'attendees': [],
    'categories': [],

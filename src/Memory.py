...
redis_url = os.environ.get("REDIS_URL", "redis://:your_password@localhost:6379/0")
    ...
    def summary_chain(self, store_message):
        try:
            ...
            prompt = ChatPromptTemplate.from_messages([
                ("system", SystemPrompt + "\n这是一段你和用户的对话记忆，对其进行总结摘要，摘要使用第一人称'我'，并且提取其中的关键信息，以如下格式返回：\n 总结摘要 | 过去对话关键信息\n例如 用户张三问候我好，我礼貌回复，然后他向我langchain的向量库信息，我回答了他今年的问题，然后他又问了比特币价格。|Langchain，向量库，比特币价格"),
                ("user", "{input}")
            ])
